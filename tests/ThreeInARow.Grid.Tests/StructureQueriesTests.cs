...
public class StructureQueriesTests : MGridTestUtility
{
    ...
    public void GivenAColumnOf8Cells_WhenQueryingConsecutiveSequencesWithLength3_ThenResultContains6Sequences()
    {
        ...
        var cellsEnumerable = this.CreateTestReadableGrid(gridData);
        ...
        Assert.Multiple(() =>
        {
            Assert.That(sequences[0].Select(c => c.Content), Is.EqualTo(new[] { "A", "B", "C" }), "First sequence should be A, B, C");
            Assert.That(sequences[1].Select(c => c.Content), Is.EqualTo(new[] { "B", "C", "D" }), "Second sequence should be B, C, D");
            Assert.That(sequences[2].Select(c => c.Content), Is.EqualTo(new[] { "C", "D", "E" }), "Third sequence should be C, D, E");
            Assert.That(sequences[3].Select(c => c.Content), Is.EqualTo(new[] { "D", "E", "F" }), "Fourth sequence should be D, E, F");
            Assert.That(sequences[4].Select(c => c.Content), Is.EqualTo(new[] { "E", "F", "G" }), "Fifth sequence should be E, F, G");
            Assert.That(sequences[5].Select(c => c.Content), Is.EqualTo(new[] { "F", "G", "H" }), "Sixth sequence should be F, G, H");
        });
    }
    ...
    public void GivenAColumnOf8Cells_WhenQueryingConsecutiveSequencesWithLength4_ThenResultContains5Sequences()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(sequences[0].Select(c => c.Content), Is.EqualTo(new[] { "A", "B", "C", "D" }), "First sequence should be A, B, C, D");
            Assert.That(sequences[1].Select(c => c.Content), Is.EqualTo(new[] { "B", "C", "D", "E" }), "Second sequence should be B, C, D, E");
            Assert.That(sequences[2].Select(c => c.Content), Is.EqualTo(new[] { "C", "D", "E", "F" }), "Third sequence should be C, D, E, F");
            Assert.That(sequences[3].Select(c => c.Content), Is.EqualTo(new[] { "D", "E", "F", "G" }), "Fourth sequence should be D, E, F, G");
            Assert.That(sequences[4].Select(c => c.Content), Is.EqualTo(new[] { "E", "F", "G", "H" }), "Fifth sequence should be E, F, G, H");
        });
    }
